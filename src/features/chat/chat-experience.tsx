...
export function ChatExperience({ profile, conversationId, initialMessages, demoMode = true }: { profile: Profile; conversationId: string; initialMessages?: DemoMessage[]; demoMode?: boolean }) {
  ...
  const send = async (event: FormEvent) => { event.preventDefault(); const text = draft.trim(); if (!text) return; const optimisticId = crypto.randomUUID(); setMessages((items) => [...items, { id: optimisticId, sender: "me", text, timestamp: "now", status: "sending" }]); setDraft(""); if (demoMode) { window.setTimeout(() => setMessages((items) => items.map((item) => item.id === optimisticId ? { ...item, status: "sent" } : item)), 450); return; } const response = await fetch("/api/messages", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ conversationId, text }) }); const result = await response.json().catch(() => null); if (!response.ok) { setMessages((items) => items.filter((item) => item.id !== optimisticId)); setDraft(text); setNotice(result?.error ?? "Message could not be sent."); return; } setMessages((items) => items.map((item) => item.id === optimisticId ? { ...item, id: result.id ?? optimisticId, status: "sent" } : item)); };
