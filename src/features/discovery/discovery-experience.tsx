        ...
        {profile ? <>
          <div className="relative h-[min(68vh,590px)] min-h-[510px]">
            ...
            <AnimatePresence initial={false} mode="popLayout">
              ...
              >
                <Portrait quadrant={profile.portrait} initials={profile.firstName} alt={profile.portrait ? `Fictional portrait of ${profile.firstName}` : `${profile.firstName}'s profile placeholder`} className="absolute inset-0 size-full" priority />
...
function ProfileDetail({ profile, vibe, viewer, onClose, onAction }: { profile: Profile; vibe: ReturnType<typeof calculateVibeScore>; viewer: DiscoveryViewer; onClose: () => void; onAction: (action: "pass" | "like") => void }) {
  ...
  <Portrait quadrant={profile.portrait} initials={profile.firstName} alt={profile.portrait ? `Fictional portrait of ${profile.firstName}` : `${profile.firstName}'s profile placeholder`} className="mt-4 aspect-[4/3] rounded-[24px]"/>
