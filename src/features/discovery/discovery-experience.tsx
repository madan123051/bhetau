...
export function DiscoveryExperience() {
  ...
  const vibe = useMemo(() => profile ? calculateVibeScore(currentUser, profile) : null, [profile]);

  const act = useCallback((action: "pass" | "like") => {
    if (!profile) return;
    ...
    if (action === "like") {
      ...
      if (result.matched) window.setTimeout(() => setMatch(profile), reduced ? 0 : 260);
  ...
  return (
    <>
          ...
          <div className="relative h-[min(68vh,590px)] min-h-[510px]">
            ...
            <AnimatePresence initial={false} mode="popLayout">
              <motion.article
                ...
                aria-label={`${profile.firstName}, ${profile.age}, ${profile.city}`}
              >
                <Portrait quadrant={profile.portrait} alt={`Fictional portrait of ${profile.firstName}`} className="absolute inset-0 size-full" priority />
                ...
                <div className="absolute inset-x-0 bottom-0 p-5 text-white">
                  <div className="flex items-end justify-between gap-4"><div><h1 className="text-4xl font-semibold tracking-[-.05em]">{profile.firstName}, {profile.age}</h1><p className="mt-1 flex items-center gap-1.5 text-sm text-white/76"><MapPin size={14}/>{profile.city} · {profile.occupation}</p></div><span className="shrink-0 rounded-full border border-white/20 bg-white/12 px-3 py-2 text-xs font-semibold backdrop-blur">{profile.intent}</span></div>
      ...
      <AnimatePresence>{detail && profile && <ProfileDetail profile={profile} vibe={vibe!} onClose={() => setDetail(false)} onAction={act}/>}</AnimatePresence>
      <AnimatePresence>{filters && <FilterSheet onClose={() => setFilters(false)}/>}</AnimatePresence>
      <AnimatePresence>{match && <MatchMoment profile={match} onClose={() => setMatch(null)}/>}</AnimatePresence>
    </>
  );
}
...
function ProfileDetail({ profile, vibe, onClose, onAction }: { profile: Profile; vibe: ReturnType<typeof calculateVibeScore>; onClose: () => void; onAction: (action: "pass" | "like") => void }) {
  ...
  <Portrait quadrant={profile.portrait} alt={`Fictional portrait of ${profile.firstName}`} className="mt-4 aspect-[4/3] rounded-[24px]"/>
  <div className="mt-5 rounded-[22px] bg-crimson/7 p-5"><p className="flex items-center gap-2 text-sm font-bold text-wine dark:text-[#ff9aac]"><Sparkles size={17}/> {vibe.score}% Vibe Match</p><p className="mt-2 text-xs leading-5 text-stone">A helpful label based on what you shared—not a scientific compatibility score.</p><ul className="mt-4 space-y-2">{vibe.reasons.map((reason) => <li key={reason} className="flex gap-2 text-sm"><CheckCircle2 size={16} className="mt-0.5 shrink-0 text-success"/>{reason}</li>)}</ul><button type="button" onClick={() => openMaya({ mode: "match_insight", action: "explain_vibe_match", currentUserProfile: { firstName: currentUser.firstName, relationshipIntention: currentUser.intent, interests: currentUser.interests, languages: currentUser.languages, promptAnswers: [] }, matchProfile: { firstName: profile.firstName, relationshipIntention: profile.intent, interests: profile.interests, languages: profile.languages, bio: profile.bio, promptAnswers: [profile.answer] } })} className="mt-4 inline-flex min-h-11 items-center gap-2 rounded-full border border-crimson/20 bg-background px-4 text-xs font-semibold text-crimson"><Sparkles size={15}/>Ask Maya why</button></div>
...
function MatchMoment({ profile, onClose }: { profile: Profile; onClose: () => void }) { return <motion.div role="dialog" aria-modal="true" aria-label="Mutual match" className="absolute inset-0 z-[60] grid place-items-center bg-[#160e12]/88 px-6 text-white backdrop-blur-lg" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: .55 }}><div className="w-full text-center"><div className="mx-auto flex w-fit items-center justify-center"><motion.div initial={{ x: 28, scale: .75 }} animate={{ x: 7, scale: 1 }} transition={{ duration: .6 }} className="z-10 size-28 overflow-hidden rounded-full border-4 border-[#160e12]"><Portrait quadrant="tl" alt="Your fictional demo profile" className="size-full"/></motion.div><motion.div initial={{ x: -28, scale: .75 }} animate={{ x: -7, scale: 1 }} transition={{ duration: .6 }} className="size-28 overflow-hidden rounded-full border-4 border-[#160e12]"><Portrait quadrant={profile.portrait} alt={`Fictional portrait of ${profile.firstName}`} className="size-full"/></motion.div></div><p className="mt-8 font-devanagari text-sm font-semibold text-[#ff9aac]">भेटौँ</p><h2 className="mt-2 text-4xl font-semibold tracking-[-.05em]">It’s a Bhetau!</h2><p className="mx-auto mt-3 max-w-xs text-sm leading-6 text-white/65">You and {profile.firstName} liked each other. Start with something you actually share.</p><Link href={`/chats/${profile.id}`} className="mt-8 flex h-14 w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-br from-[#ff5a72] to-[#d72c55] font-semibold"><MessageCircle size={19}/>Say hi</Link><Button variant="ghost" onClick={onClose} className="mt-3 w-full text-white hover:bg-white/8">Keep exploring</Button></div></motion.div>; }
