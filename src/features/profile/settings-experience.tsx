...
function SettingsSection({ title, children }: { title: string; children: React.ReactNode }) { return <section className="mx-5 mt-6"><h2 className="mb-2 px-1 text-[11px] font-bold uppercase tracking-[.13em] text-stone">{title}</h2><div className="divide-y overflow-hidden rounded-[22px] border bg-surface">{children}</div></section>; }
function SettingToggle({ icon: Icon, label, hint, value, busy, onChange }: { icon?: LucideIcon; label: string; hint?: string; value: boolean; busy: boolean; onChange: () => void }) { return <button type="button" role="switch" aria-checked={value} aria-busy={busy} disabled={busy} onClick={onChange} className="flex min-h-14 w-full items-center gap-3 px-4 py-2 text-left disabled:opacity-65">{Icon && <Icon size={18} className="text-stone"/>}<span className="flex-1"><span className="block text-sm font-medium">{label}</span>{hint && <span className="mt-0.5 block text-[11px] leading-4 text-stone">{hint}</span>}</span><span className={`relative h-7 w-12 shrink-0 rounded-full transition ${value ? "bg-crimson" : "bg-foreground/15"}`}><span className={`absolute top-1 size-5 rounded-full bg-white shadow transition ${value ? "left-6" : "left-1"}`}/></span></button>; }
function LinkRow({ href, icon: Icon, label }: { href: string; icon: LucideIcon; label: string }) { return <Link href={href} prefetch={false} className="flex min-h-16 items-center gap-3 px-4"><Icon size={19} className="text-stone"/><span className="flex-1 text-sm font-medium">{label}</span><ChevronRight size={17} className="text-stone"/></Link>; }
